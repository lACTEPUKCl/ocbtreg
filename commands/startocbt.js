...
const startocbt = new SlashCommandBuilder()
  .setName("startocbt")
  .setDescription("Возобновить регистрацию на OCBT.")
  .setDefaultMemberPermissions(PermissionFlagsBits.MuteMembers);

const execute = async (interaction) => {
  try {
    await toggleOCBTButtons(interaction, false);
  } catch (error) {
  ...
  try {
    ...
    return interaction.reply({
      content: disable
        ...
        : "Регистрация успешно возобновлена. Кнопки включены.",
      ephemeral: true,
    });
  } catch (error) {
    return interaction.reply({
...
export default { data: startocbt, execute };
